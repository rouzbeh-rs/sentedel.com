const API = 'https://rrsanjabi-sentedel-edi-phi-api.hf.space/api';
...
async function checkHealth() {
    setStatus('Connecting to inference API…');
    try {
        const res = await fetch(`${API}/health`);
        const data = await res.json();
        if (data.ok) {
            window.__modelReady = true;
            setStatus('Ready. Paste EDI and click Detect & redact.', 'ok');
        ...
        setStatus(
            data.error
                ? `API reachable, but model is not loaded: ${data.error}`
                : 'API reachable, but model is not loaded. Try again in a few minutes.',
            'error',
        );
    } catch (err) {
        window.__modelReady = false;
        console.error(err);
        setStatus(
            'Cannot reach the inference API. The Space may be starting up — wait a minute and refresh.',
            'error',
        );
    }
}
